...
function getTargetHitRate(reels, rtp) {
  const baseByReels = { 3: 0.22, 5: 0.15, 7: 0.11 };
  const base = baseByReels[reels] || 0.15;
  const adjustment = (rtp - 0.96) * 0.9;
  return Math.max(0.04, Math.min(0.35, base + adjustment));
}
...
function getWinSymbol(reels, rtp) {
  ...
  const premiumBoost = Math.max(0, (rtp - 0.94) * 20);

  const adjusted = symbols
    .filter((s) => s.icon !== "⭐")
    .map((s) => {
      let weight = s.weight;
      if (s.payout >= 15) weight += premiumBoost;
      return { ...s, weight };
    });

  return Math.random() < targetHitRate ? weightedPick(adjusted) : null;
}
...
function maybeBonus(bet) {
  ...
  state.bonusMeter = 0;
  const bonus = Math.round((bet * (8 + Math.random() * 20)) * 100) / 100;
  state.balance += bonus;
...
spinBtn.addEventListener("click", () => spinOnce(true));
autoBtn.addEventListener("click", () => autoSpin(100, false));
turboBtn.addEventListener("click", () => autoSpin(500, true));
